...
public class RangerAuthorizationCoprocessor extends RangerAuthorizationCoprocessorBase implements AccessControlService.Interface, CoprocessorService {
	...
	private final String messageTemplate = "evaluateAccess: exiting: user[%s], Operation[%s], access[%s], families[%s], verdict[%s]";
    ...
    //KEY:RegionNode+User+Operation+Action+ColumnFamily.
	private static Map<String, ColumnFamilyAccessResult> globColumnFamilyAccessResultCache = new HashMap<String, ColumnFamilyAccessResult>();
	private static Map<String, HbaseAccessCache> globhBaseAccessCache = new HashMap<String, HbaseAccessCache>();
	private static RangerAuthorizationCoprocessor instance = null;
	private static String hbaseAccessCachelock = "baseAccessCachelock";
	private static final String timeFormat = "yyyy-MM-dd HH:mm:ss";

	public void startUpdatePoliciesTask() {
		instance = this;
		Timer timer = new Timer();
		timer.schedule(new UpdatePoliciesTask(), 2*1000, hbasePlugin.getPollingIntervalMs());//在
	}
	...
	ColumnFamilyAccessResult evaluateAccess(String operation, Action action, final RegionCoprocessorEnvironment env,
		...
		String cacheKey = operation + getRemoteAddress() + userName + access + table + clusterName + getColumnFamilyKey(familyMap);
		result = getGlobalHbaseAccessFromCache(cacheKey);
		if (result == null) {
			Map<String, Set<String>> families = getColumnFamilies(familyMap);
			HbaseAccessCache hbaseAC = new HbaseAccessCache();
			hbaseAC.setOperation(operation);
			hbaseAC.setClusterName(clusterName);
			hbaseAC.setAccess(access);
			hbaseAC.setFamilies(families);
			hbaseAC.setRemoteAddr(getRemoteAddress());
			hbaseAC.setTable(table);
			hbaseAC.setUserName(userName);
			SimpleDateFormat sdf =   new SimpleDateFormat( timeFormat );
			hbaseAC.setUser(user);
			String updateTime = sdf.format(new Date());
			hbaseAC.setUpdateTime(updateTime);
			result = getColumnFamilyAccessResult(user, operation, getRemoteAddress(), access, table, clusterName, families);
			putGlobHbaseAccessToCache(cacheKey, result,hbaseAC);
		}
		else {
			updateGlobBaseAccessCache(cacheKey);
		}
		return result;
		// let's create a session that would be reused.  Set things on it that won't change.
	}

	private ColumnFamilyAccessResult getColumnFamilyAccessResult(User user, String operation, String remoteIp, String access,
												String table, String clusterName,
		...
		if (families == null || families.isEmpty()) {
			...
			if (LOG.isDebugEnabled()) {
				String message = String.format(messageTemplate, _userUtils.getUserAsString(user), operation, access, families.toString(), result.toString());
				LOG.debug(message);
			}
			return result;
		} else {
  ...
  public void preSetNamespaceQuota(final ObserverContext<MasterCoprocessorEnvironment> ctx,
    ...
    private String getColumnFamilyKey(final Map<byte[], ? extends Collection<?>> familyMap){
		String columnFamilyAndColumn = "";
		for (Map.Entry<String, Set<String>> columnFamily : getColumnFamilies(familyMap).entrySet()) {
	...
	private void putGlobHbaseAccessToCache(String key, ColumnFamilyAccessResult result, HbaseAccessCache hbaseAccessCache) {
		synchronized (hbaseAccessCachelock) {
			globColumnFamilyAccessResultCache.put(key, result);
			globhBaseAccessCache.put(key, hbaseAccessCache);
		}
	}
	private void updateGlobBaseAccessCache(String key) {
		HbaseAccessCache hbaseAccessCache = globhBaseAccessCache.get(key);
		SimpleDateFormat sdf =   new SimpleDateFormat( timeFormat );
		String updateTime = sdf.format(new Date());
		if (hbaseAccessCache != null) {
			synchronized (hbaseAccessCachelock) {
				hbaseAccessCache.setUpdateTime(updateTime);
			}
		}
	}

	private ColumnFamilyAccessResult getGlobalHbaseAccessFromCache(String sKey) {
		ColumnFamilyAccessResult mapRet = null;
		synchronized (hbaseAccessCachelock) {
			mapRet = globColumnFamilyAccessResultCache.get(sKey);
		}
		return mapRet;
	}
	private void removeInvalidPolicies(long lCurrentTime){
		SimpleDateFormat sdf =   new SimpleDateFormat( timeFormat );
		for (Map.Entry<String, HbaseAccessCache> clumnFAResultCache : globhBaseAccessCache.entrySet()) {
			String sKey = clumnFAResultCache.getKey();
			HbaseAccessCache hbaseAccessCache = clumnFAResultCache.getValue();
			try {
				long from = sdf.parse(hbaseAccessCache.getUpdateTime()).getTime();
				int days = (int) ((lCurrentTime - from)/(1000 * 60 * 60 * 24));
				if (days > 1) {
					synchronized (hbaseAccessCachelock) {
						globhBaseAccessCache.remove(sKey);
					}
				}
	...
	private void updateGlobHbaseAccess() throws AccessDeniedException {
		long lCurrentTime = new Date().getTime();
		removeInvalidPolicies(lCurrentTime);
		if (hbasePlugin.isHaveNewPolicy() && !hbasePlugin.getUpdateStatus()) {
			for (Map.Entry<String, HbaseAccessCache> clumnFAResultCache : globhBaseAccessCache.entrySet()) {
				String sKey = clumnFAResultCache.getKey();
				HbaseAccessCache hbaseAccessCache = clumnFAResultCache.getValue();
				ColumnFamilyAccessResult mapRet = getColumnFamilyAccessResult(hbaseAccessCache.getUser(), hbaseAccessCache.getOperation(), hbaseAccessCache.getRemoteAddr(),
						hbaseAccessCache.getAccess(), hbaseAccessCache.getTable(), hbaseAccessCache.getClusterName(),
						hbaseAccessCache.getFamilies());
				synchronized (hbaseAccessCachelock) {
					globColumnFamilyAccessResultCache.put(sKey, mapRet);
					hbasePlugin.setHaveNewPolicy(false);
				}
			}
		}
	}

	class UpdatePoliciesTask extends TimerTask {
		@Override
		public void run() {
			if (RangerAuthorizationCoprocessor.instance != null) {
				try {
					RangerAuthorizationCoprocessor.instance.updateGlobHbaseAccess();
				} catch (Exception err) {
					LOG.error(err);
				}
			}
